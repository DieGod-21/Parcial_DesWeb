 ...
 *         description:
 *           type: string
 *           example: "La humanidad se transforma en robots gigantes para defender la costa este de los monstruos que surgen del fondo del mar."
 *         Ubicacion:
 *           type: string
 *           example: "POPCINEMA"
 */
 ...
 *     responses:
 *       200:
 *         description: Arreglo de películas
 */
// ===== Serie III – GET lista =====
 ...
 *     responses:
 *       200:
 *         description: {"codError":"200","msgRespuesta":"Registro Insertado"}
 */
// ===== Serie I – POST inserta =====
router.post("/", async (req, res) => {
  ...
  if (missing.length) {
    return res.status(400).json({ codError:"400", msgRespuesta:"Solicitud inválida", camposFaltantes: missing });
  }
  ...
  try {
    ...
    await pool.request()
      ...
      .input("Estado", sql.Bit, Boolean(b.Estado))
 ...
 *     responses:
 *       200:
 *         description: {"codError":"200","msgRespuesta":"Registro Actualizado"}
 *       404:
 *         description: {"codError":"404","msgRespuesta":"Registro no encontrado"}
 *       400:
 *         description: {"codError":"400","msgRespuesta":"Solicitud inválida o mal formada"}
 */
// ===== Serie II – PUT actualiza por ?imdbID= =====
router.put("/", async (req, res) => {
  ...
  const missing = required.filter(k => b[k] === undefined);
  if (missing.length) {
    return res.status(400).json({ codError:"400", msgRespuesta:"Solicitud inválida o mal formada", camposFaltantes: missing });
  }
